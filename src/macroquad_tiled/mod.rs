...
impl Map {
    ...
    pub fn draw_tiles(&self, layer: &str, dest: Rect, source: impl Into<Option<Rect>>) {
        ...
        for y in source.y as u32..source.y as u32 + source.h as u32 {
            for x in source.x as u32..source.x as u32 + source.w as u32 {
                if let Some(tile) = &layer.data.get((y * layer.width + x) as usize).unwrap_or(&None) {
    ...
    pub fn draw_imglayer(&self, layer: &str, dest: Rect, source: Option<Rect>) {
        ...
        let dest_width_frac = img_texture.width() / (self.raw_tiled_map.width * self.raw_tiled_map.tilewidth) as f32;
        let dest_height_frac = img_texture.height() / (self.raw_tiled_map.height * self.raw_tiled_map.height) as f32;
...
impl<'a> TilesIterator<'a> {
    fn new(layer: &'a Layer, rect: Rect) -> Self {
        ...
        TilesIterator { layer, rect, current }
    }
}
...
pub fn load_map(
    ...
    for layer in &map.layers {
        ...
        let find_tileset = |tile: u32| {
            ...
            map_tilesets
                .iter()
                .find(|tileset| tile >= tileset.firstgid && tile < tileset.firstgid + tileset.tilecount)
        };
